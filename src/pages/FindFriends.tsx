...
export default function FindFriends({ onBack }: FindFriendsProps) {
    ...
    const fetchEventAttendees = async () => {
      ...
      try {
        ...
        const { data: profilesData, error: profilesError } = await supabase
          .from('profiles')
          .select('user_id, avatar_url, notes, notes_visible, find_friends_visible, instagram_url, phone, interest, relationship_status')
          .in('user_id', userIds)
  ...
  const getRelationshipBadge = (status: string) => {
    const statusConfig = {
      solteiro: { color: "bg-green-500/20 text-green-400", label: "Solteiro(a)", emoji: "😊" },
      namorando: { color: "bg-pink-500/20 text-pink-400", label: "Namorando", emoji: "💑" },
      casado: { color: "bg-purple-500/20 text-purple-400", label: "Casado(a)", emoji: "💍" },
      relacionamento_aberto: { color: "bg-orange-500/20 text-orange-400", label: "Relacionamento Aberto", emoji: "🌈" },
      preferencia_nao_informar: { color: "bg-gray-500/20 text-gray-400", label: "Não informado", emoji: "🤐" }
    };
