...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can view registrations for platform events'
  ) THEN
    CREATE POLICY "Users can view registrations for platform events"
      ON public.event_registrations
      FOR SELECT
      USING (platform_event_id IS NOT NULL);
  END IF;
END $$;
